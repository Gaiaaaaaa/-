...
<div>
  <div class="app-container calendar-list-container">
    ...
    <!-- <el-form>
      <el-form-item>
        <el-button size="small" type="primary" @click="showEditManage">管理营业部变更</el-button>
        <el-button size="small" type="primary" @click="showEditDialog">管理营业部变更记录</el-button>
      </el-form-item>
    </el-form> -->
    ...
    :data="list" v-loading.body="listLoading" border stripe fit style="width: 100%">
      ...
      <el-table-column fixed="right"  align="center" min-width="180px" label="操作"
                       v-if="$store.getters.buttons.includes('btnAlterationDepartmentChange') || $store.getters.buttons.includes('btnAlterationDepartmentChangeRecord')">
        <template scope="scope">
          <el-button  @click="showEditManage(scope.row)" type="text"
                      v-if="$store.getters.buttons.includes('btnAlterationDepartmentChange')">变更</el-button>
          <el-button  @click="showEditDialog(scope.row)" type="text"
                      v-if="$store.getters.buttons.includes('btnAlterationDepartmentChangeRecord')">记录</el-button>
        </template>
      </el-table-column>
...
import { getdepartmentList, getOrderStatus } from "@/api/order";
  ...
  created() {
    ...
    Utils.getDictData("ITEM_STATUS", data => {
      ...
      for (var n = 0; n < this.statusOptions.length; n++) {
        ...
        if (this.statusOptions[n].dictDataKey == "002") {
          this.statusOptions[n].disabled = true;
        }
