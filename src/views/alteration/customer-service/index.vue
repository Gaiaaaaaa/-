...
<div>
  ...
  <el-form class="form-container" :model="listQuery" ref="searchForm" label-width="100px" :inline="true">
      ...
      <el-form-item>
        <el-button size="small" icon="search" type="primary" v-loading="searchLoading" @click="search">查询</el-button>
        <el-button size="small" @click="resetForm('searchForm')">重置</el-button>
      </el-form-item>
    </el-form>
    <!-- <el-form>
      <el-form-item>
        <el-button size="small" type="primary" @click="showEditManage">客服变更</el-button>
        <el-button size="small" type="primary" @click="showEditDialog">客服变更记录</el-button>
      </el-form-item>
    </el-form> -->
    ...
    <el-table ref="singleTable" tooltip-effect="dark"
      ...
      <el-table-column fixed="right"  align="center" min-width="180px" label="操作"
                       v-if="$store.getters.buttons.includes('btnAlterationCustomerServiceChange') || $store.getters.buttons.includes('btnAlterationCustomerServiceChangeRecord')">
        <template scope="scope">
          <el-button  @click="showEditManage(scope.row)" type="text"
                      v-if="$store.getters.buttons.includes('btnAlterationCustomerServiceChange')" :disabled="!scope.row.businessId" >变更</el-button>
          <el-button  @click="showEditDialog(scope.row)" type="text"
                      v-if="$store.getters.buttons.includes('btnAlterationCustomerServiceChangeRecord')">记录</el-button>
        </template>
      </el-table-column>
    ...
    <div class="block pagination-container" style="text-align: right;">
      ...
      <!-- 提示信息 -->
      <i v-if="NoticeiShow" @click="open"></i>

     <edit-modal v-if="showModal" @close="showModal = false"  @refresh="getList"  @getList="getList"  :itemData="currentRow"></edit-modal>
    <updateHis-modal v-if="showTransfer" @close="showTransfer = false" @getList="getList" :ipiecesNo="currentRow.ipiecesNo"></updateHis-modal>
  ...
  export default {
    ...
    methods: {
      ...
      showEditManage(item){
        ...
        if(this.currentRow.customerServiceId == null) {
          this.showModal = false
          Utils.error('当前没有客服，不能进行变更')
        }else{
          this.showModal = true
        }
      },
